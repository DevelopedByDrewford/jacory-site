...
const VIEWPORTS = {
  desktop: { w: 1440, h: 1100 },
  mobile: { w: 390, h: 900 },
};

function AboutSimulation({ url, mode }) {
  ...
  const sectionPadTop = isDesktop ? 120 : 96;

  // About grid: 0.9fr 1.1fr, gap 80px
  const wrapW = w - hPad * 2;
  const gap = isDesktop ? 80 : 0;
  const totalFr = isDesktop ? 2.0 : 1;
  const portraitW = isDesktop ? (wrapW - gap) * (0.9 / totalFr) : wrapW;
  const portraitH = portraitW * (5 / 4);
        ...
        <div style={{ paddingTop: sectionPadTop, paddingLeft: hPad, paddingRight: hPad }}>
          <div style={{ maxWidth: 1280, margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 34 }}>
              <span style={{ fontWeight: 600, fontSize: 11, letterSpacing: '0.32em', textTransform: 'uppercase', color: '#d9a55b' }}>About</span>
              <span style={{ fontSize: 11, letterSpacing: '0.2em', color: '#6b6256' }}>03</span>
            ...
            <h2 style={{
              ...
              lineHeight: 1.28,
              margin: `0 0 ${isDesktop ? 72 : 48}px`,
              letterSpacing: '-0.01em',
              maxWidth: '36ch',
            }}>
              He was told the world would get smaller.{' '}
              <em style={{ fontStyle: 'italic', color: '#d9a55b' }}>So he made it bigger</em>{' '}
              — one page, one swing, one talk at a time.
            </h2>
            ...
            <div style={{
              ...
              {/* Portrait */}
              <div style={{ position: 'relative', width: portraitW, height: portraitH }}>
                {/* Gold frame accent */}
                <div style={{
                  position: 'absolute',
                  top: -14, right: -14,
                  width: '70%', height: '70%',
                  border: '1px solid rgba(217,165,91,0.35)',
                  zIndex: 0,
                  pointerEvents: 'none',
                }} />
                <img
              ...
              {isDesktop && (
                <div style={{ paddingTop: 8 }}>
                  <p style={{ color: '#d8cdba', fontSize: 17, lineHeight: 1.75, margin: '0 0 22px', maxWidth: '52ch' }}>
                    <strong style={{ color: '#f4ede0', fontWeight: 500 }}>Jacory Wiley is a children's author and motivational speaker from Houston, Texas.</strong>{' '}
                    He became visually impaired as a boy — an age when most kids are still learning what they want to be.
                  </p>
                  <p style={{ color: '#d8cdba', fontSize: 17, lineHeight: 1.75, margin: '0 0 22px', maxWidth: '52ch' }}>
                    In 2022 he found <strong style={{ color: '#f4ede0', fontWeight: 500 }}>beep baseball</strong> — the game played by athletes who are blind —
                    and discovered that the diamond didn't care what he couldn't see.
                  </p>
                  <p style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontStyle: 'italic', fontSize: 32, color: '#d9a55b', margin: '32px 0 0' }}>
                    Jacory Wiley
                  </p>
                </div>
              )}
...
function PreviewModal({ img, onClose }) {
  ...
  return (
    ...
    >
      ...
      <AboutSimulation url={img.url} mode={mode} />
...
export default function ManageAbout() {
  ...
  const fetchImages = async () => {
    const snap = await getDocs(query(collection(db, 'aboutImages'), orderBy('addedAt', 'desc')));
  ...
  const handleAdd = async () => {
    ...
    setStatus('');
    try {
      await addDoc(collection(db, 'aboutImages'), { url, altText: altInput.trim(), addedAt: Date.now() });
      await fetchImages();
      setUrlInput('');
      setAltInput('');
  ...
  const setLive = async (img) => {
    setStatus('');
    try {
      await setDoc(doc(db, 'config', 'about'), { activeUrl: img.url, activeAlt: img.altText || '' });
  ...
  const deleteImage = async (img) => {
    ...
    try {
      await deleteDoc(doc(db, 'aboutImages', img.id));
      if (activeUrl === img.url) {
        await setDoc(doc(db, 'config', 'about'), { activeUrl: '', activeAlt: '' });
        setActiveUrl('');
      }
      await fetchImages();
      setStatus('Removed.');
  ...
  const handleEditSave = async (img) => {
    ...
    if (!url) return;
    try {
      await updateDoc(doc(db, 'aboutImages', img.id), { url, altText: editAlt.trim() });
      if (activeUrl === img.url) {
        await setDoc(doc(db, 'config', 'about'), { activeUrl: url, activeAlt: editAlt.trim() });
        setActiveUrl(url);
      }
      await fetchImages();
      setEditingId(null);
          ...
          {images.map((img) => {
            ...
            return (
              <div key={img.id} style={{
                ...
                {/* 4:5 thumbnail */}
                <div onClick={() => setPreviewImg(img)} style={{ position: 'relative', paddingTop: '125%', background: '#f0ede8', cursor: 'pointer' }}>
