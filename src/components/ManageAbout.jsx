...
export default function ManageAbout() {
  ...
  const handleAdd = async () => {
    ...
    setStatus('');
    try {
      await addDoc(collection(db, 'aboutImages'), { url, addedAt: Date.now() });
      await fetchImages();
      setUrlInput('');
  ...
  const setLive = async (img) => {
    setStatus('');
    try {
      await setDoc(doc(db, 'config', 'about'), { activeUrl: img.url });
  ...
  const deleteImage = async (img) => {
    ...
    try {
      ...
      if (activeUrl === img.url) {
        await setDoc(doc(db, 'config', 'about'), { activeUrl: '' });
        setActiveUrl('');
      }
      await fetchImages();
      setStatus('Removed.');
